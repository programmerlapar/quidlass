...
### Fixed
- **SSR Hydration**: Removed render-time `window.devicePixelRatio` reads from `LiquidGlass` so the hidden canvas renders stable dimensions during SSR and initial client hydration
- **Canvas DPI Sync**: Moved DPR handling into a post-mount effect and refreshed shader generation when DPR changes so the effect still sharpens correctly after hydration
- **Regression Coverage**: Added an SSR hydration regression test covering server render, client hydration, and post-mount DPR resize behavior
...
[1.2.2]: https://github.com/programmerlapar/quidlass/compare/v1.2.1...v1.2.2
